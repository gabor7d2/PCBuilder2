package net.gabor7d2.pcbuilderold.utils;
...
import static net.gabor7d2.pcbuilderold.utils.FileUtils.FileOperationResult.*;
...
public class FileUtils {
    ...
    public static FileOperationData<String> copyFileFromJar(String pathInJar, String destPath, boolean destPathIsDirectory) {
        // Need leading / to path in jar for getResourceAsStream to work
        if (pathInJar.charAt(0) != File.separatorChar) pathInJar = "/".concat(pathInJar);

        // replace all \ to / (because path is inside jar file)
        ...
        String destFilePath;
        if (destPathIsDirectory) {
            destFilePath = destPath + File.separator + new File(pathInJar).getName();
        } else {
     ...
     *                  This must be the exact file that should be written, e.g. new File("./defaults/desserts/cakes.jpg")
     * @return The result of the operation
     */
